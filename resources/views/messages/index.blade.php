...
<div class="messages-container">
    ...
    <div class="messages-tabs">
        <a href="{{ route('messages.index') }}" class="active">Ontvangen</a>
        <a href="{{ route('messages.sent') }}">Verzonden</a>
    </div>

    @if($unreadCount > 0)
        <div class="messages-actions">
            <form method="POST" action="{{ route('messages.mark-all-as-read') }}">
                @csrf
                <button type="submit" class="btn btn-secondary">
                    Markeer alle als gelezen ({{ $unreadCount }})
                </button>
            </form>
        </div>
    @endif
    ...
    @if($messages->count() > 0)
        <div class="messages-list">
            @foreach($messages as $message)
                <x-message-card :message="$message" type="received" />
            @endforeach
        </div>
